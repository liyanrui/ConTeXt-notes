...
若你熟悉 \TEX\ Live，推荐使用它提供的 \CONTEXT\ 版本，其功能较为稳定，且国内有镜像网站，在线安装更为便捷。若是新安装 \TEX\ Live，在安装时可直接选择 \type{ConTeXt scheme} 或在 \type{collections} 列表中选择 \type{ConTeXt and packages}。若你的系统中已安装 \TEX\ Live，只是无 \CONTEXT，可使用 TeXLive 包管理器 \type{tlmgr} 安装 \type{collection-context} 即可。
