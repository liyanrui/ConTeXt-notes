...
\stopsample
\sample[todo-list]{竖向盒子和横向盒子}{\externalfigure[10/vbox-and-hbox.pdf]}
...
\noindent 结果为 \framed[location=low]{\type{\framed[location=low]{...}}}。上述示例中的 \type{\type} 用于排版代码，它会自动将字体切换为等宽字体（\type{\tt}）原样显示它所接受的文字。顺便多言，若是排版一段代码，可使用 \type{\starttyping[option=TEX] ... \stoptyping}。
...
\starttyping[option=TEX]
\placetable[force]{box0 的几何信息}{\tfx
  \starttabulate[|c|c|c|]
  \HL
