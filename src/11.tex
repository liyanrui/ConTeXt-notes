...
\stoptyping
\defineoverlay[晃晃][\useMPgraphic{随机晃动的矩形}]
\dontleavehmode\kern .25em\framed[frame=off,background=晃晃]{光辉岁月}
...
\stoptyping
\startMPcode
string s; s = "\color[darkred]{\bf 江山如此多娇}";
draw textext(s);
draw thetextext(s, (4cm, 0));
draw textext(s) scaled 1.5 shifted (8cm, 0) ;
draw textext(s) scaled 1.5 rotated 45 shifted (12cm, 0);
\stopMPcode
...
picture p; p := image(fill a withcolor darkgray;
...
numeric w, h; w := bbwidth(p); h := bbheight(p);
numeric offset; offset := 5mm;
path q; q := fullsquare xscaled (w + offset) yscaled (h + offset) shifted center p;
fill q withcolor darkgray;
draw q withpen pencircle scaled 2pt withcolor darkred;
draw p;
\stopMPcode
...
\stoptyping
\midaligned{\externalfigure[11-flowchart-1.pdf]}
\blank
...
\stoptyping
\midaligned{\externalfigure[11-flowchart-2.pdf]}
\blank
...
\stoptyping
\midaligned{\externalfigure[11-flowchart-3.pdf]}
...
\stoptyping
\leftaligned{\externalfigure[11-flowchart-4.pdf]}
\blank
...
path p; p := fullsquare xscaled 4cm yscaled 1cm;
draw p randomized 3mm
       withcolor transparent (0, .5 randomized .25, red)
       withpen pencircle scaled 2pt;
\stopuseMPgraphic
\startoverlay
  {\useMPgraphic{一个矩形}}
...
\defineoverlay
  [叠叠]
  [\startoverlay
   {\useMPgraphic{一个矩形}} {\useMPgraphic{一个矩形}} {\useMPgraphic{一个矩形}}
   \stopoverlay]
\stoptyping
\starttyping[option=TEX]
...
\defineoverlay
  ...
  [\startoverlay
   ...
   \stopoverlay]
\defineframed[foo][frame=off,background={叠叠}]
\dontleavehmode\kern .25em\foo{迎接光辉岁月}

\type{\framed} 的也可以将多个 overlay 作为背景，以下代码与上例等效：
...
\stoptyping
\midaligned{\externalfigure[11-watermark.png][width=.6\textwidth]}
...
也许你意犹未尽，甚至觉得我语焉不详。切莫怪我，我原本仅仅是介绍如何使用 \MetaPost\ 绘制一个边框受随机扰动的盒子。

不过，在大致掌握了本章所述的内容的基础上，关于 \MetaPost\ 更多的知识，特别是宏的定义方面，可阅读它的手册\cite[mpman]。此外，\ConTeXt\ 开发者 Hans Hagen 所写的 \MetaFun\ 手册除了讲述 \MetaFun\ 之外也大量介绍了 \MetaPost\ 的基本知识和技巧，该手册在你的 \ConTeXt\ 系统中，使用以下命令查找：
