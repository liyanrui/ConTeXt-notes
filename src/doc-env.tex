...
\setupinteraction[state=start,
  ...
  title={ConTeXt 蹊径},
  author={李延瑞},
  keyword={TeX, ConTeXt, LMTX, MkIV, LuaTeX, LuaMetaTeX}]
...
\setuphead
  [title,chapter]
  [style=\bfb,header=empty,footer=empty,before=,after={\blank[1cm]}]
...
\setuptyping
  [before={\startblueframe[width=\textwidth]},
    after={\stopblueframe},escape=yes]
\definetyping
  [narrowtyping]
  [option=TEX,before={\startblueframe[width=fit]},after={\stopblueframe},escape=yes]
...
\definebuffer[samplecode]
\def\mysample[#1]#2#3{%
  \placeSample[force][#1]{#2}{%
    \hbox to \textwidth{%
      \hbox to .45\textwidth{\getsamplecode}\hfill\externalfigure[#3][maxwidth=.45\textwidth]%
    }%
  }%
}
% 修改 MPcode 代码部分高亮颜色
